...
def x_to_pixel(x):
    return round((x/6.5 + 0.5)*map_width)

def pixel_to_x(pixel):
    return (pixel/map_width - 0.5) * 6.5

def y_to_pixel(y):
    return round((y/3.25 + 0.5)*map_length) - map_length/6

def pixel_to_y(pixel):
    return ((pixel + map_length/6)/map_length - 0.5) * 3.25
...
ax = plt.gca()
cbar = ax.figure.colorbar(hm, ax=ax, fraction=0.025, pad=0.04)