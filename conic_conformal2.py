...
def y_to_pixel(y):
    return round((y/6 + 0.5)*map_length) + 0.5*map_length

def pixel_to_y(pixel):
    return ((pixel - 0.5*map_length)/map_length - 0.5) * 6
...
hm = plt.imshow(area_dist, cmap='RdPu', interpolation='nearest', alpha=0.7, norm=colors.LogNorm(vmax=10**2))  
ax = plt.gca()
